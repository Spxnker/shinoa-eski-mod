...
if(!message.member.roles.cache.has(ayarlar.guildHammer.vmuteHammer) && !message.member.hasPermission("ADMINISTRATOR")) return embedCreator("#4D0A0A", `Bu komutu kullanmak için yeterli yetkin bulunmuyor.`,message,[true,5000])
...
let mList = await Database.find({activity: true, victimID: victim.id, Type: "VOİCE-MUTE"})
if (mList.length <= 0) return embedCreator("#4D0A0A", `**Belirttiğin kişi herhangi bir mute cezasına sahip değil.**`,message,[true,5000])
