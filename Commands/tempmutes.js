...
let data = await Database.find({guildID: message.guild.id, victimID: victim.id, activity: true, Type: "MUTE"})
...
victim.roles.add(ayarlar.guildRoles.muteRole)

if(logKanal) logKanal.send(embed.setDescription(`${victim} (\`${victim.id}\`) üyesi metin kanallarında susturuldu.\n**•** Mute Başlangıç: \`${muteAtılma}\`\n**•** Mute Bitiş: \`${muteBitiş}\`\n**•** Süre: \`${yaziSure}\`\n\n**•** Sebep: \`${reason}\``).setFooter(`Ceza numarası: #${count}`).setTimestamp());
